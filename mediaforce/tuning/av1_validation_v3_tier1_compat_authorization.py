...
AV1_VALIDATION_V3_TIER1_COMPAT_REQUEST_SCHEMA = (
    "mediaforce.av1_cold_start_v3_tier1_compat_request"
)
AV1_VALIDATION_V3_TIER1_COMPAT_GRANT_SCHEMA = (
    "mediaforce.av1_cold_start_v3_tier1_compat_grant"
)
AV1_VALIDATION_V3_TIER1_COMPAT_SCHEMA_VERSION = 1
AV1_VALIDATION_V3_TIER1_COMPAT_REQUEST_CONTRACT_VERSION = "av1vt1cpr1"
AV1_VALIDATION_V3_TIER1_COMPAT_GRANT_CONTRACT_VERSION = "av1vt1cpg1"
AV1_VALIDATION_V3_TIER1_COMPAT_AUTHORITY = "av1_v3_tier1_compat_probe_execution"
_OWNER_PRINCIPAL_RE = re.compile(r"owner-[a-z0-9]{8,32}\Z")
_SHA256_RE = re.compile(r"sha256:[0-9a-f]{64}\Z")


class AV1ValidationV3Tier1CompatAuthorizationError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class AV1ValidationV3Tier1CompatRequest:
    request_id: str
    protocol_id: str
    ...
    def __post_init__(self) -> None:
        if not self.request_id.startswith("av1vtier1compatrequest3_"):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility request ID is invalid"
            )
        for value in (
            ...
            if not _SHA256_RE.fullmatch(value):
                raise AV1ValidationV3Tier1CompatAuthorizationError(
                    "AV1 v3 Tier 1 compatibility request digest is invalid"
                )
        requested_at = _parse_timestamp(self.requested_at)
        valid_until = _parse_timestamp(self.valid_until)
        if valid_until <= requested_at:
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility request expiration is invalid"
            )
        semantic = self.semantic_payload()
        if self.request_id != av1_validation_v3_id("tier1compatrequest", semantic):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility request ID does not match its payload"
            )
        if self.payload_sha256 != _payload_sha256(
            {"request_id": self.request_id, **semantic}
        ):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility request digest does not match its payload"
            )
        assert_av1_cold_start_public_payload_safe(self.to_payload())

    def semantic_payload(self) -> dict[str, Any]:
        return {
            "schema": AV1_VALIDATION_V3_TIER1_COMPAT_REQUEST_SCHEMA,
            "schema_version": AV1_VALIDATION_V3_TIER1_COMPAT_SCHEMA_VERSION,
            "contract_version": AV1_VALIDATION_V3_TIER1_COMPAT_REQUEST_CONTRACT_VERSION,
            "authority": AV1_VALIDATION_V3_TIER1_COMPAT_AUTHORITY,
            "gate": "A0",
            "tier": "none",
...
@dataclass(frozen=True, slots=True)
class AV1ValidationV3Tier1CompatGrant:
    grant_id: str
    request_id: str
    ...
    def __post_init__(self) -> None:
        if not self.grant_id.startswith("av1vtier1compatgrant3_"):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility grant ID is invalid"
            )
        if (
            not self.request_id.startswith("av1vtier1compatrequest3_")
            or not _SHA256_RE.fullmatch(self.request_payload_sha256)
            or not _OWNER_PRINCIPAL_RE.fullmatch(self.owner_principal)
        ):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility grant binding is invalid"
            )
        authorized_at = _parse_timestamp(self.authorized_at)
        valid_until = _parse_timestamp(self.valid_until)
        if valid_until <= authorized_at:
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility grant expiration is invalid"
            )
        semantic = self.semantic_payload()
        if self.grant_id != av1_validation_v3_id("tier1compatgrant", semantic):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility grant ID does not match its payload"
            )
        if self.payload_sha256 != _payload_sha256(
            {"grant_id": self.grant_id, **semantic}
        ):
            raise AV1ValidationV3Tier1CompatAuthorizationError(
                "AV1 v3 Tier 1 compatibility grant digest does not match its payload"
            )
        assert_av1_cold_start_public_payload_safe(self.to_payload())

    def semantic_payload(self) -> dict[str, Any]:
        return {
            "schema": AV1_VALIDATION_V3_TIER1_COMPAT_GRANT_SCHEMA,
            "schema_version": AV1_VALIDATION_V3_TIER1_COMPAT_SCHEMA_VERSION,
            "contract_version": AV1_VALIDATION_V3_TIER1_COMPAT_GRANT_CONTRACT_VERSION,
            "authority": AV1_VALIDATION_V3_TIER1_COMPAT_AUTHORITY,
            "gate": "A0",
            "tier": "none",
            "diagnostic_only": True,
            "fixture_scope": "deterministic_synthetic_public_only",
            "compat_probe_execution_authorized": True,
            **_false_authority_fields(),
            "request_id": self.request_id,
...
def build_av1_validation_v3_tier1_compat_request(
    *,
    ...
    if _parse_timestamp(valid_until) > _parse_timestamp(plan.valid_until):
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility request cannot outlive its plan"
        )
    ...
    request_id = av1_validation_v3_id("tier1compatrequest", semantic)
    return AV1ValidationV3Tier1CompatRequest(
...
def assert_av1_validation_v3_tier1_compat_request_active(
    protocol: AV1ValidationProtocolV3,
    plan: AV1ValidationV3QualificationPlan,
    ...
    *,
    ...
    if request != build_av1_validation_v3_tier1_compat_request(
        protocol=protocol,
        plan=plan,
        ...
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility request is not bound to its plan"
        )
    ...
    if checked_at < _parse_timestamp(request.requested_at) or checked_at >= _parse_timestamp(
        ...
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility request is not active"
        )


def build_av1_validation_v3_tier1_compat_grant(
    *,
    protocol: AV1ValidationProtocolV3,
    plan: AV1ValidationV3QualificationPlan,
    request: AV1ValidationV3Tier1CompatRequest,
    ...
    if _parse_timestamp(valid_until) > _parse_timestamp(request.valid_until):
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility grant cannot outlive its request"
        )
    ...
    grant_id = av1_validation_v3_id("tier1compatgrant", semantic)
    return AV1ValidationV3Tier1CompatGrant(
        grant_id=grant_id,
...
def assert_av1_validation_v3_tier1_compat_grant_active(
    protocol: AV1ValidationProtocolV3,
    plan: AV1ValidationV3QualificationPlan,
    request: AV1ValidationV3Tier1CompatRequest,
    ...
    *,
    ...
    if grant.request_id != request.request_id or grant.request_payload_sha256 != request.payload_sha256:
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility grant is not bound to its request"
        )
    checked_at = _parse_timestamp(as_of)
    if checked_at < _parse_timestamp(grant.authorized_at) or checked_at >= _parse_timestamp(
        ...
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility grant is not active"
        )


def serialize_av1_validation_v3_tier1_compat_request(
    request: AV1ValidationV3Tier1CompatRequest,
) -> bytes:
    return canonical_json_bytes(request.to_payload()) + b"\n"


def serialize_av1_validation_v3_tier1_compat_grant(
    grant: AV1ValidationV3Tier1CompatGrant,
) -> bytes:
    return canonical_json_bytes(grant.to_payload()) + b"\n"


def load_av1_validation_v3_tier1_compat_request(
    path: Path,
) -> AV1ValidationV3Tier1CompatRequest:
    return _load(
        path,
        av1_validation_v3_tier1_compat_request_from_payload,
        serialize_av1_validation_v3_tier1_compat_request,
    )


def load_av1_validation_v3_tier1_compat_grant(
    path: Path,
) -> AV1ValidationV3Tier1CompatGrant:
    return _load(
        path,
...
def av1_validation_v3_tier1_compat_request_from_payload(
    ...
    if value != request.to_payload():
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility request fields are invalid"
        )
    return request


def av1_validation_v3_tier1_compat_grant_from_payload(
    payload: Mapping[str, Any],
) -> AV1ValidationV3Tier1CompatGrant:
    value = object_dict(payload)
    grant = AV1ValidationV3Tier1CompatGrant(
    ...
    if value != grant.to_payload():
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility grant fields are invalid"
        )
    return grant
...
def _load(path: Path, parser: Any, serializer: Any) -> Any:
    ...
    try:
        ...
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility authorization artifact is invalid"
        ) from exc
    if raw != serializer(item):
        raise AV1ValidationV3Tier1CompatAuthorizationError(
            "AV1 v3 Tier 1 compatibility authorization bytes are not canonical"
        )
    return item
