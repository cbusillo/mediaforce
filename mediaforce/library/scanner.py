...
def scan_library(connection: DBClient, config: MediaforceConfig, prefixes: list[str] | None = None,
    ...
    connection.exec_driver_sql(
        ...
        (
            ...
            roots_json,
            scope,
            json.dumps(normalized_prefixes) if normalized_prefixes else None,
        ),
    )
    connection.commit()
            ...
            if row and row["size_bytes"] == stat_result.st_size and row["mtime_ns"] == stat_result.st_mtime_ns:
                connection.exec_driver_sql(
                    ...
                    SET last_scan_id = ?,
                        last_seen_at = ?,
                        updated_at   = ?,
                        status       = CASE WHEN status = 'missing' THEN 'discovered' ELSE status END
                    WHERE source_path = ?
                    """,
    ...
    connection.exec_driver_sql(
        ...
        (
            ...
            completed_at,
            stats.total_seen,
            stats.reprobed + stats.discovered,
            stats.unchanged,
            scan_id,
        ),
    )
...
def _flush_scan_progress(
    ...
    connection.exec_driver_sql(
        ...
        WHERE scan_id = ?
        """,
        (timestamp(), stats.total_seen, stats.reprobed + stats.discovered, stats.unchanged, scan_id),
    )
    connection.commit()
    return 0
