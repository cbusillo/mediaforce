...
from mediaforce.web.routes.queues import (
    ...
    register_child_recovery_routes,
)
    ...
    [
        ...
        (
            [
                ...
                (b"origin", b"https://unrelated.example"),
            ],
            403,
        ),
        (
...
def test_recovery_rejects_cross_origin_and_simple_form_requests(
...
def test_recovery_does_not_trigger_unrelated_artifact_cleanup() -> None:
    ...
    for path in (CHILD_RECOVERY_PREVIEW_PATH, CHILD_RECOVERY_APPLY_PATH):
        assert not _request_triggers_periodic_cleanup("POST", path)
        assert not _request_triggers_periodic_cleanup("POST", path + "/")
    assert _request_triggers_periodic_cleanup("POST", "/api/settings")


def test_recovery_accepts_same_origin_json_request() -> None:
    app = FastAPI()
    action = Mock(return_value={"ok": True})
    register_child_recovery_routes(app, recover_children_action=action)
    endpoint = next(route.endpoint for route in app.routes if route.path == CHILD_RECOVERY_APPLY_PATH)
    req = request({"parent_job_id": "p", "child_ids": ["a"], "token": "t"})
    req.scope["headers"].append((b"origin", b"http://localhost:8777"))
    assert asyncio.run(endpoint(req)).status_code == 200
    action.assert_called_once_with("p", ["a"], "t")
