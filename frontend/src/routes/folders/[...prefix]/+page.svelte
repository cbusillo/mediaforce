<script lang="ts">
	...
	type EncodeStatusTone = 'live' | 'queued' | 'warning' | 'neutral';
	...
	type PolicyWorkbenchSection = { title: string; rows: PolicyWorkbenchRow[] };
	type WorkbenchStat = { label: string; value: string; detail?: string };
	...
	function formatBooleanCopy(value: boolean | null | undefined): string {
	...
	function flattenPolicy(policyValue: FolderPolicy | null | undefined): Record<string, unknown> {
	...
	function policyRowLabel(path: string): string {
		const labels: Record<string, string> = {
	...
	function formatPolicyValue(path: string, value: unknown): string {
		if (value == null) return 'n/a';
		...
		if (path === 'video.quality_metric')
			return resolveMetricLabel(String(value), folder.metric_support);
		return String(value);
	}

	function workbenchSection(path: string): string {
	...
	function encodeStatusTone(status: string): EncodeStatusTone {
	...
	function formatDateTimeCopy(value: string | null | undefined): string {
		const trimmed = String(value ?? '').trim();
		if (!trimmed) return '';
	...
	const sampleSetupCopy = $derived.by(() =>
		hasCalibration
			? 'The latest note, tuning summary, and run readout stay together here so you can steer the next pass without restating the whole story.'
			: 'This run keeps the current gate context, measures one representative draft, and unlocks the review deck below.'
	);
	...
	const proposalWorkbenchRows = $derived.by(() => {
		...
		return paths.map((path) => {
			const current = formatPolicyValue(path, currentPolicy[path]);
			const draft = formatPolicyValue(path, previewPolicy[path]);
			return {
				path,
...
<div class="page-stack">
		...
		<Panel class="status-strip-panel" padding="0.95rem 1rem">
			<div class="status-strip">
				<div class="section-copy-block">
					...
					<p class="muted-copy">The catalog snapshot is updating so this view stays aligned with the latest media state.</p>
				</div>
				<p class="status-strip-meta">Started {String(status.folder_scan_job.started_at ?? status.folder_scan_job.created_at ?? '')}</p>
			</div>
		</Panel>
	{/if}
	...
	{#if calibrationJob && (status.calibration_status === 'queued' || status.calibration_status === 'running')}
		<Panel class="status-strip-panel accent-strip" padding="0.95rem 1rem">
			<div class="status-strip">
				<div class="section-copy-block">
					<p class="eyebrow-copy">{calibrationJob.mode === 'full' ? 'Proof encode' : 'Calibration'}</p>
				...
				<p class="status-strip-meta">
					Action {String(calibrationJob.action ?? '')} · Host {String(calibrationJob.host?.label ?? '')}
				</p>
			</div>
		</Panel>
	{/if}
						...
						<div class="host-picker-shell">
							<div class="section-copy-block">
								...
								<p class="muted-copy host-section-copy">
									Pick the machine for this representative pass. The chosen host stays visible in the action bar below.
								</p>
							</div>
									...
									>
											...
											<span class="muted-copy compact-host-meta">
												{[compactScheduleCopy(hostCard.runtime), hostCapacityCopy(hostCard.runtime)]
													.filter(Boolean)
													.join(' · ') || hostCard.detail || 'No runtime detail'}
											</span>
										</button>
								{/each}
							</div>
								...
								<div class="thread-inline-head">
									<div class="section-copy-block">
										...
										<p class="muted-copy thread-history-copy">
											Every operator note and bench reply stays in this one scrolling conversation.
										</p>
									</div>
							...
							<div class:stale={pendingProposalNeedsRefresh} class="proposal-shell diagnosis-shell">
								...
								<div class="diagnosis-grid">
									<section class="proposal-workbench-card diagnosis-copy-card">
										...
										{#if previewSubmission}
											<p class="inline-gate-copy proposal-warning-copy">
												...
												Bench received {previewSubmission.note || 'your latest request'} and is preparing a fresh reply.
											</p>
										{/if}
									</section>
									...
									<section class="proposal-workbench-card">
										...
										{#if !proposalWorkbenchSections.length}
											<p class="muted-copy">
												This draft keeps the current policy. Review the evidence below before deciding whether to ask again.
											</p>
										{/if}
									...
									<div class="proposal-workbench-grid diagnosis-disclosure-grid">
												...
												<div class="proposal-memory-shell">
													{#each workbenchMemoryEntries as entry (entry.title + entry.summary)}
														<div class="proposal-memory-card">
															<p class="proposal-memory-title">{entry.title || 'Prior approved tuning note'}</p>
															{#if entry.summary}
										...
										<section class="proposal-workbench-card">
											...
											{#if pendingProposalTrace?.prompt_version}
												<p class="muted-copy"><strong>Prompt version:</strong> {pendingProposalTrace.prompt_version}</p>
											{/if}
						...
						<div class="review-block review-evidence-block">
								...
								<div class="review-player-shell-block">
									<div class="review-player-head">
										<div>
											...
											<p class="muted-copy">
												Play either clip and the other follows. The lower player is the retained AV1 draft preview from this run.
											</p>
										</div>
									</div>
									...
									<div class="review-player-stack review-player-columns">
										<div class="review-player-panel">
											<div class="review-player-meta">
												...
												<p class="muted-copy">Browser review render from the representative source file.</p>
											</div>
											<video
										...
										<div class="review-player-panel draft-review-player-panel">
											<div class="review-player-meta">
												...
												<p class="muted-copy">The retained preview from the sampled draft you are deciding on now.</p>
											</div>
											<video
							...
							{:else if calibration.review_media_ready}
								<div class="legacy-review-note">
									...
									<p class="muted-copy">
										This draft still has review artifacts, but it predates the paired browser player media. Run a fresh sample if you want synced playback here before approving it.
									</p>
								</div>
							{/if}
					...
					<aside class="review-side-column review-approval-column">
						...
						{#if predictedOutputSizeBytes > 0}
							<div class="review-block estimate-block compact-estimate-block">
								...
								<h3 class="review-block-title">{formatGiB(predictedOutputSizeBytes, 2)} predicted output</h3>
								<p class="muted-copy">Representative-file estimate for the current draft before you approve this folder.</p>
								<div class="pill-row">
									<Pill label={`Source ${formatGiB(Number(sampleItem.source_size_bytes ?? 0), 2)}`} variant="neutral" />
									...
									{#if calibrationSampleResult?.chosen_crf != null}
										<Pill label={`CRF ${Number(calibrationSampleResult.chosen_crf).toFixed(1)}`} variant="ghost" />
									{/if}
								</div>
							</div>
						{/if}
						...
						<details class="reference-disclosure" open={false}>
							...
							<div class="reference-disclosure-grid">
								<div class="review-block representative-block compact-reference-block">
									...
									<div class="representative-path-shell" title={representativePath || 'No representative file yet'}>
										...
										<div class="representative-meta-row">
											...
											{/if}
											<Pill label={`Size ${formatGiB(Number(sampleItem.source_size_bytes ?? 0), 2)}`} variant="neutral" />
											{#if representativeVideoBitrate}
												<Pill label={`Video ${representativeVideoBitrate}`} variant="neutral" />
											{/if}
											<Pill label={`Length ${formatTimestamp(Number(sampleItem.duration_seconds ?? 0))}`} variant="neutral" />
										</div>
									</div>
								</div>
